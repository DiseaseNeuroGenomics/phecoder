...
from typing import Dict, Any, Optional, Tuple, Iterable, Set, List
import numpy as np
import pandas as pd
...
def _vectorized_metrics_for_model(
    ...
    """
    if sim.empty:
        return (pd.DataFrame(columns=["model","phecode","k","n_considered","n_gold_pos","AP@k","P@k","R@k"]),
    ...
    for kk in Ks:
        ...
        at_k = sim.loc[sim["idx"].eq(take_k), ["phecode","idx","tp_cum","n_gold_pos","prec_hit_cum"]].copy()
        ...
        Rk = np.where(denom_gold > 0, at_k["tp_cum"] / denom_gold, 0.0).astype("float32")
        ...
        block = pd.DataFrame({
            ...
            "k":        (at_k["n_considered"].to_numpy() if k_out is None else np.full(len(at_k), k_out, dtype=np.int32)),
    ...
    metrics_df = (pd.concat(blocks, ignore_index=True)
                  if blocks else
                  pd.DataFrame(columns=["model","phecode","k","n_considered","n_gold_pos","AP@k","P@k","R@k"]))

    curves_df = None
    if include_curves:
        ladder = sim[["phecode","idx","tp_cum","n_gold_pos"]].copy()
        ladder["precision"] = (ladder["tp_cum"] / ladder["idx"]).astype("float32")
        ladder["recall"]    = np.where(ladder["n_gold_pos"] > 0, ladder["tp_cum"] / ladder["n_gold_pos"], 0.0).astype("float32")
        groups = []
...
def _warn_missing_codes(
    ...
    by_phe = gold_df.groupby("phecode")["icd_code"].apply(lambda s: set(s) - icd_universe)
    ...
    missing_icds_df = pd.DataFrame(records) if records else pd.DataFrame(columns=["phecode", "missing_icd"])

    # --- Entire phecodes with no overlap ---
    phe_with_overlap = {
        phe for phe, icds in gold_df.groupby("phecode")["icd_code"]
    ...
    if not missing_icds_df.empty:
        ...
        for phe, n in by_phe_counts.items():
            some = missing_icds_df.loc[missing_icds_df["phecode"] == phe, "missing_icd"].head(10).tolist()
            msgs.append(f"{phe}: {n} missing (e.g., {some}{'...' if n > 10 else ''})")
    if fully_excluded_phecodes:
        msgs.append(f"Fully excluded phecodes: {fully_excluded_phecodes[:10]}{'...' if len(fully_excluded_phecodes) > 10 else ''}")

    if msgs:
        print("[evaluate] excluded ground-truth ICDs/phecodes not in ICD dataframe:\n  " + "\n  ".join(msgs), flush=True)
    ...
    missing_phe_df = pd.DataFrame({"phecode": fully_excluded_phecodes, "missing_icd": None, "missing_type": "full"})

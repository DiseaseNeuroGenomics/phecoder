...
def _precompute_arrays(
    ...
    recall = (
        (cum_tp / float(n_pos_total)).astype(np.float64)
        if n_pos_total > 0
        else np.zeros_like(precision)
    )
...
def _ap_at_k(
    precision: np.ndarray, pos_idx: np.ndarray, n_pos_total: int, k: Optional[int]
) -> float:
    """Rank-aware AP. If k is provided, consider positives with rank ≤ k."""
